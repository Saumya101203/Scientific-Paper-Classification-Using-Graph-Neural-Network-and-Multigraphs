...
# =============================================================================
SUBSET_RATIO = 0.4  # Exactly matching the initial Transformer attempt
NUM_EPOCHS = 300
HIDDEN_CHANNELS = 128
DROPOUT = 0.5
...
if __name__ == "__main__":
    print(f"🚀 Initializing SAGE Control Experiment on: {DEVICE}")
    ...
    # 2. INJECT 1524 BOSS LEVEL FEATURES 
    ...
    model_args = {"num_layers": 2, "in_channels": 1524, "out_channels": out_channels, "hidden_channels": HIDDEN_CHANNELS, "dropout": DROPOUT}
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        if epoch % 5 == 0:  # Check validation more frequently since SAGE is fast
            val_acc = test(model, data, data['paper'].val_idx)
            if val_acc > best_acc:
                best_acc = val_acc
                best_epoch = epoch
                torch.save(model.state_dict(), "models/best_sage_subset.pth")
    ...
    model.load_state_dict(torch.load("models/best_sage_subset.pth", weights_only=True))
    test_acc = test(model, data, data['paper'].test_idx)
    print("="*50)
    print(f"🏆 FINAL SAGE TEST ACCURACY (40% Subset): {test_acc:.4f}")
    print("="*50)