...
from torch_geometric.utils import homophily, dropout_edge # Added dropout_edge


# --- 2. 🚀 NEW: Focal Loss Class ---
class FocalLoss(nn.Module):
    """
    A standard Focal Loss implementation.
    ...
    def forward(self, inputs, targets):
        # 'inputs' are raw logits from your model (shape: [N, C])
        # 'targets' are ground truth labels (shape: [N])
...
# --- 3. 🚀 FIXED SAGE CLASS (Copied from analyze_errors.py) ---
...
# 4. 🚀 COPIED WeightedHeteroWrapper (Copied from analyze_errors.py)
...
# 5. 🚀 COPIED get_model (Copied from analyze_errors.py)
...
# 6. 🚀 MODIFIED train and test functions
def train(model, data, train_idx, optimizer, loss_fn, device):
    model.train()
    ...
    # --- ❗ Use Focal Loss ---
    loss = loss_fn(out[train_idx], data["paper"].y[train_idx].squeeze())
...
@torch.no_grad()
def test(model, data, idx, dataset, loss_fn, out=None):
    model.eval()
    if out is None:
        out = model(data.x_dict, data.adj_t_dict)['paper']
        
    # --- ❗ Use Focal Loss for validation/test loss ---
    loss = loss_fn(out[idx], data["paper"].y[idx].squeeze())
...
def main():
    ...
    print(f"Starting Focal Loss run...")
    ...
    # Use original train/val/test indices
    ...
    # Good defaults are alpha=0.25, gamma=2.0
    ...
    for run in range(params.get("runs", 1)): # Use 'runs' from config, default to 1
        ...
        for epoch in tqdm(range(params["epochs"]), desc=f"Run {run:02d}"):
            # Pass loss_fn to train
            train_loss, out = train(model, data, train_idx, optimizer, loss_fn, DEVICE)
            
            # Pass loss_fn to test
            val_acc, val_loss = test(model, data, val_idx, params["dataset"], loss_fn, out=out)
            scheduler.step()
        ...
        model_path = f"models/focal_loss_run{run}_best.pth" # Temp save for testing
        torch.save(model.state_dict(), model_path)
        model.load_state_dict(torch.load(model_path))
        
        test_acc, test_loss = test(model, data, test_idx, params["dataset"], loss_fn)
